...
const starts = async (kurr = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('PutraXd Base', {
		font: 'Standard',
...
console.log(color('> HARI ','silver'), color(`${tampilHari}\n`,'mediumseagreen'))
	kurr.browserDescription = [ 'Putra Xd ', 'ubuntu', '3.0' ]
	...
	try {
		...
		if (anu.action == 'add') {
			...
			try {
				...
				sendButImage(anu.jid, teks, `Create By PutraXd Ofc`, img,but = [{buttonId:`apaaja`, 
			...
			try {
				...
				sendButImage(anu.jid, teks, `Create By PutraXd Ofc`, img,but = [{buttonId: `Hello World!`, buttonText: {displayText: `AMIN`}, type: 1}], options = {contextInfo: {mentionedJid: [num, htod]}})
